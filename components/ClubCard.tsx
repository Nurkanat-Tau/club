...
export function ClubCard({ club, members }: { club: Club; members: number }) {
  return (
    <Link href={`/c/${club.slug}`} className="card flex min-w-0 gap-4 p-4 transition hover:border-brand">
      ...
      <div className="min-w-0 flex-1">
        ...
        <p className="line-clamp-2 break-words text-sm text-muted [overflow-wrap:anywhere]">{club.description}</p>
        <p className="mt-1 line-clamp-2 text-xs text-muted">
          {club.schedule_text} · {countLabel(members, MEMBERS)}
        </p>
      </div>
    </Link>
  );
